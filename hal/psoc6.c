...
#define EXT_FLASH_ROW_SIZE ROW_SIZE
...
#define SMIF_HF_CLK_DIVIDER (CY_SYSCLK_CLKHF_DIVIDE_BY_4)
#define SMIF_INIT_TRY_COUNT (10U)
#define SMIF_INIT_RETRY_DELAY_MS (500U)
 ...
 */
static GPIO_PRT_Type* qspi_SS_Port = GPIO_PRT11;
static uint32_t qspi_SS_Pin        = 2u;
static en_hsiom_sel_t qspi_SS_Mux  = P11_2_SMIF_SPI_SELECT0;

static GPIO_PRT_Type* qspi_D3_Port = GPIO_PRT11;
static uint32_t qspi_D3_Pin        = 3u;
static en_hsiom_sel_t qspi_D3_Mux  = P11_3_SMIF_SPI_DATA3;

static GPIO_PRT_Type* qspi_D2_Port = GPIO_PRT11;
static uint32_t qspi_D2_Pin        = 4u;
static en_hsiom_sel_t qspi_D2_Mux  = P11_4_SMIF_SPI_DATA2;

static GPIO_PRT_Type* qspi_D1_Port = GPIO_PRT11;
static uint32_t qspi_D1_Pin        = 5u;
static en_hsiom_sel_t qspi_D1_Mux  = P11_5_SMIF_SPI_DATA1;

static GPIO_PRT_Type* qspi_D0_Port = GPIO_PRT11;
static uint32_t qspi_D0_Pin        = 6u;
static en_hsiom_sel_t qspi_D0_Mux  = P11_6_SMIF_SPI_DATA0;

static GPIO_PRT_Type* qspi_SCK_Port = GPIO_PRT11;
static uint32_t qspi_SCK_Pin        = 7u;
static en_hsiom_sel_t qspi_SCK_Mux  = P11_7_SMIF_SPI_CLK;
...
cy_en_smif_status_t hal_ext_flash_init(void)
{
    ...
    st = Cy_SMIF_Init(SMIF0, &QSPI_config, SMIF_INIT_TIMEOUT_US, &smif_context);
    ...
    try_count       = SMIF_INIT_TRY_COUNT;
    do {
        st = Cy_SMIF_MemInit(SMIF0, smif_blk_config, &smif_context);
        try_count--;
        if (st != CY_SMIF_SUCCESS) {
            Cy_SysLib_Delay(SMIF_INIT_RETRY_DELAY_MS);
        }
...
{
    Cy_SMIF_MemDeInit(SMIF0);
    Cy_SMIF_Disable(SMIF0);
...
int RAMFUNCTION ext_flash_write(uintptr_t address, const uint8_t* data, int len)
{
    ...
    uint8_t tmp_buffer[EXT_FLASH_ROW_SIZE];
    cy_rslt_t status;
    ...
    if (need_update) {
        ...
        if (hal_flash_erase(TMP_SECTOR_INFO_ADDR, ROW_SIZE) != 0) {
            wolfBoot_printf("ext_flash_write: hal_flash_erase failed at "
                            "0x%08lx (row size=%lu)\n",
                            (unsigned long)TMP_SECTOR_INFO_ADDR,
                            (unsigned long)ROW_SIZE);
            goto error;
        }

        /* Erase the temporary sector */
        status = Cy_SMIF_MemEraseSector(SMIF0, smif_blk_config->memConfig[0],
                                        SWAP_TMP_SECTOR_ADDR - base_address,
                                        erase_size, &smif_context);
        if (status != CY_SMIF_SUCCESS) {
            wolfBoot_printf(
                "ext_flash_write: Cy_SMIF_MemEraseSector failed for tmp sector "
                "(addr=0x%08lx, erase_size=%lu, status=0x%08lx)\n",
                (unsigned long)(SWAP_TMP_SECTOR_ADDR - base_address),
                (unsigned long)erase_size, (unsigned long)status);
            goto error;
        }

        /* Backup source sector to temporary location row by row */
        for (offset = 0; offset < erase_size; offset += ROW_SIZE) {
            status = Cy_SMIF_MemRead(SMIF0, smif_blk_config->memConfig[0],
                                     sector_start - base_address + offset,
                                     tmp_buffer, ROW_SIZE, &smif_context);
            if (status != CY_SMIF_SUCCESS) {
                wolfBoot_printf(
                    "ext_flash_write: Cy_SMIF_MemRead failed "
                    "(src=0x%08lx, len=%lu, status=0x%08lx)\n",
                    (unsigned long)(sector_start - base_address + offset),
                    (unsigned long)ROW_SIZE, (unsigned long)status);
                goto error;
            }

            status =
                Cy_SMIF_MemWrite(SMIF0, smif_blk_config->memConfig[0],
                                 SWAP_TMP_SECTOR_ADDR - base_address + offset,
                                 tmp_buffer, ROW_SIZE, &smif_context);
            if (status != CY_SMIF_SUCCESS) {
                wolfBoot_printf("ext_flash_write: Cy_SMIF_MemWrite failed "
                                "(dst=0x%08lx, len=%lu, status=0x%08lx)\n",
                                (unsigned long)(SWAP_TMP_SECTOR_ADDR -
                                                base_address + offset),
                                (unsigned long)ROW_SIZE, (unsigned long)status);
                goto error;
            }
        ...
        if (hal_flash_write(TMP_SECTOR_INFO_ADDR, (const void*)&info,
            ...
            wolfBoot_printf("ext_flash_write: hal_flash_write failed updating "
                            "tmp_sector_info "
                            "(addr=0x%08lx, size=%lu)\n",
                            (unsigned long)TMP_SECTOR_INFO_ADDR,
                            (unsigned long)sizeof(tmp_sector_info));
            goto error;
        }
        ...
        status = Cy_SMIF_MemEraseSector(SMIF0, smif_blk_config->memConfig[0],
                                        ...
                                        &smif_context);
        if (status != CY_SMIF_SUCCESS) {
            wolfBoot_printf("ext_flash_write: Cy_SMIF_MemEraseSector failed "
                            "for source sector "
                            "(addr=0x%08lx, erase_size=%lu, status=0x%08lx)\n",
                            (unsigned long)(sector_start - base_address),
                            (unsigned long)erase_size, (unsigned long)status);
            goto error;
        }
    }
         ...
         */
        for (i = 0; i < len; i += EXT_FLASH_ROW_SIZE) {
            status = Cy_SMIF_MemRead(SMIF0, smif_blk_config->memConfig[0],
                                     address - base_address + i, tmp_buffer,
                                     EXT_FLASH_ROW_SIZE, &smif_context);
            if (status != CY_SMIF_SUCCESS) {
                wolfBoot_printf(
            ...
            if ((tmp_buffer[0] != FLASH_ERASED_VALUE) ||
                (memcmp(&tmp_buffer[1], &tmp_buffer[0],
                        EXT_FLASH_ROW_SIZE - 1) != 0)) {
                /* Non-erased data found, erase the containing sector */
                status = Cy_SMIF_MemEraseSector(
                    SMIF0, smif_blk_config->memConfig[0],
...
{
    tmp_sector_info info = *(tmp_sector_info*)TMP_SECTOR_INFO_ADDR;
