 ...
 *
 * External flash has a larger minimum erase size (0x40000 bytes) than internal
 * flash rows. Since read-modify-erase-write cannot use SRAM as a temporary
 * buffer due to size constraints, we use a dedicated sector in external flash
 * for temporary storage during sector updates.
 */
#define SWAP_TMP_SECTOR_ADDR                                                   \
    (WOLFBOOT_PARTITION_UPDATE_ADDRESS + CY_FLASH_SIZE)
...
int RAMFUNCTION hal_flash_write(uint32_t address, const uint8_t* data, int len)
{
    ...
    if (address != row_start) {
        ...
        write_len = ((ROW_SIZE - offset) < len) ? (ROW_SIZE - offset) : len;
        memcpy(&psoc6_write_buffer[offset], data, write_len);

        /* Program the updated data into flash row */
        ret = Cy_Flash_ProgramRow(row_start, (const uint32_t*)psoc6_write_buffer);
        if (ret) {
            wolfBoot_printf("hal_flash_write: ProgramRow failed at 0x%08X ret=%d\n", row_start, ret);
            return ret;
        }
    ...
    while (len > 0) {
        ...
        write_len = (len >= ROW_SIZE) ? ROW_SIZE : len;
...
int RAMFUNCTION hal_flash_erase(uint32_t address, int len)
{
    ...
    while (len > 0) {
        if ((address % FLASH_SECTOR_SIZE == 0) && (len >= FLASH_SECTOR_SIZE)) {
 ...
 * (0x40000 bytes) than the row size used by wolfBoot. Implements a temporary
 * sector backup mechanism to preserve existing data during partial sector
 * writes.
 *
 * Algorithm:
...
int RAMFUNCTION ext_flash_write(uintptr_t address, const uint8_t* data, int len)
{
    ...
    base_address = smifBlockConfig.memConfig[0]->baseAddress;
    erase_size   = smifBlockConfig.memConfig[0]->deviceCfg->eraseSize;
    ...
    if (need_update) {
        ...
        status = Cy_SMIF_MemEraseSector(SMIF0, smifBlockConfig.memConfig[0],
        ...
        for (offset = 0; offset < erase_size; offset += ROW_SIZE) {
            status = Cy_SMIF_MemRead(SMIF0, smifBlockConfig.memConfig[0],
            ...
            status =
                Cy_SMIF_MemWrite(SMIF0, smifBlockConfig.memConfig[0],
        ...
        for (i = 0; i < len; i += EXT_FLASH_ROW_SIZE) {
            ...
            if ((tmp_buffer[0] != FLASH_ERASED_VALUE) ||
                ...
                /* Non-erased data found, erase entire source sector */
                status = Cy_SMIF_MemEraseSector(
                    SMIF0, smifBlockConfig.memConfig[0],
                    sector_start - base_address, erase_size, &smif_context);
                if (status != CY_SMIF_SUCCESS) {
                    wolfBoot_printf("ext_flash_write: SMIF erase failed for sector 0x%08lX\n", (unsigned long)sector_start);
                    goto error;
                }
                break;
            }
    ...
    /* Write the row data */
    status = Cy_SMIF_MemWrite(SMIF0, smifBlockConfig.memConfig[0],
...
int RAMFUNCTION ext_flash_read(uintptr_t address, uint8_t* data, int len)
{
    ...
    return Cy_SMIF_MemRead(SMIF0, smifBlockConfig.memConfig[0], offset, data,
                           len, &smif_context);
}
 ...
 * flash minimum erase size is much larger (0x40000 bytes).
